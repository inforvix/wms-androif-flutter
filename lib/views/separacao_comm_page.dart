...
class SeparacaoEcommPage extends StatefulWidget {
  final String? Pedido;
  SeparacaoEcommPage({super.key, required this.Pedido});

  @override
  State<SeparacaoEcommPage> createState() => _SeparacaoEcommPageState();
}

class _SeparacaoEcommPageState extends State<SeparacaoEcommPage> {
  final ButtonStyle style =
      ElevatedButton.styleFrom(fixedSize: const Size(240, 50), textStyle: const TextStyle(fontSize: 20));
  final DataGridController _dataGridController = DataGridController();

  List<Ecommerce> listExp = [];
  List<Ecommerce> filter = [];
  ...
  void atualizaLista() {
    var retorno = EcommerceControler().getByPedido(widget.Pedido!);
    {
      retorno.then((value) => {
            listExp = value,
  ...
  void filtrar(int tipo) {
    if (tipo == 2) {
      ...
      filter.addAll(listExp);
      if (tipo == 0) {
        filter.removeWhere((item) => item.exp_quantidade_separada! == item.exp_quantidade_separar);
      }
      filter.sort((a, b) {
      ...
      if (produto.isEmpty) {
        ...
        if (filter.isEmpty) {
          ...
          if (ler >= (lido + double.parse(qtdLida))) {
            ...
            exp.exp_quantidade_separada = (exp.exp_quantidade_separada ?? 0.0) + double.parse(qtdLida);
            ...
            alertDialog(context, 'Quantidade do produto maior do que o necessário!');
            clearTextProduto();
            escolheFocus();
            beepErro();
          }
                      ...
                      child: SfDataGrid(
                        ...
                        onCellLongPress: (details) {
                          ...
                          if (row != null) {
                            ...
                            fieldTextQtd.text = row.getCells()[3].value.toString();
                        ...
                        columns: <GridColumn>[
                          GridColumn(
                              width: larguraDisponivel * (gFlEndereco == 'S' ? 0.32 : 0.54),
                          ...
                          GridColumn(
                              width: larguraDisponivel * (gFlEndereco == 'S' ? 0.14 : 0.22),
...
class ExpedicaoDataSource extends DataGridSource {
  ...
  ExpedicaoDataSource({required List<Ecommerce> expedicaoData}) {
    _expediaoData = expedicaoData
        .map<DataGridRow>((e) => DataGridRow(cells: [
              DataGridCell<String>(columnName: 'pro_codigo', value: e.pro_codigo),
              DataGridCell<String>(columnName: 'exp_endereco', value: e.exp_endereco),
              DataGridCell<String>(columnName: 'exp_caixa', value: e.exp_caixa),
              DataGridCell<double>(columnName: 'exp_quantidade_separar', value: e.exp_quantidade_separar),
              DataGridCell<double>(columnName: 'exp_quantidade_separada', value: e.exp_quantidade_separada),
            ]))
        .toList();
  }
