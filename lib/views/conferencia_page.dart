...
class ConferenciaPage extends StatefulWidget {
  final String? Pedido;
  ConferenciaPage({super.key, required this.Pedido});
...
class _ConferenciaPageState extends State<ConferenciaPage> {
  ...
  List<Conferencia> listConf = [];
  List<Conferencia> filter = [];
  double ler = 0.0;
  double conf = 0.0;

  late ExpedicaoDataSource expedicaoDataSource =
      ExpedicaoDataSource(expedicaoData: listConf);
  ...
  void atualizaLista() {
    var retorno = ExpedicaoControler().getByPedidoConferencia(widget.Pedido!);
    {
      retorno.then((value) => {
            listConf = value,
  ...
  void filtrar(int tipo) {
    ...
    filter.sort((a, b) {
      var cmp = (a.exp_quantidade_conferida ?? 0.0)
          .compareTo(a.exp_quantidade_separar);
      if (cmp == 0) {
        return 1;
    ...
    void lerProduto(String produto) {
      if (produto.isEmpty) {
        ...
        if (filter.isEmpty) {
          ...
          if (ler > lido) {
            conf.exp_quantidade_conferida =
                (conf.exp_quantidade_conferida ?? 0.0) + 1;
              ...
              child: Scrollbar(
                child: SfDataGrid(
                  ...
                  columns: <GridColumn>[
                    ...
                    GridColumn(
                        width: larguraDisponivel * 0.3,
                        columnName: 'exp_quantidade_conferida',
                        label: Container(
                            //padding: EdgeInsets.all(5.0),
                            alignment: Alignment.center,
...
class ExpedicaoDataSource extends DataGridSource {
  ...
  ExpedicaoDataSource({required List<Conferencia> expedicaoData}) {
    _expediaoData = expedicaoData
        .map<DataGridRow>((e) => DataGridRow(cells: [
              ...
              DataGridCell<double>(
                  columnName: 'exp_quantidade_conferida',
                  value: e.exp_quantidade_conferida),
            ]))
        .toList();
  }
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    Color getRowBackgroundColor() {
      ...
      final double lido = row.getCells()[2].value;
      if (ler == lido) {
